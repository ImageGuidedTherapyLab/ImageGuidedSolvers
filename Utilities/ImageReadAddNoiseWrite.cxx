...
int main( int argc, char ** argv )
{
  ...
  IteratorType    outputIt(inputImage, inputImage->GetRequestedRegion());

  //ImageType::Pointer   noiseImage = ImageType::New();
  //noiseImage->SetRegions(   inputImage->GetRequestedRegion() );
  //noiseImage->Allocate();
  //PixelType data = noiseImage->GetBufferPointer();
  //ImageRegionIterator<ImageType>  noiseIt(noiseImage,
  //                                        noiseImage->GetRequestedRegion());
  ...
  srand ( seed );
  sleep(1); // sleep for a second to get a new seed next time

  int noisemax = 10; 

  while( !outputIt.IsAtEnd() ) 
    {
    //outputIt.Set( inputIt1.Get() + noiseIt.Get() );
    outputIt.Set( outputIt.Get() + rand() % (noisemax+1) - 0.5 * noisemax );
    ++outputIt;
    //++noiseIt;
    }
