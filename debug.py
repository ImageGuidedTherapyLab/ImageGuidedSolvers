...
for o, a in opts:
    if o in ("-i", "--idb"):
        ...
        try: 
           ...
           iproc = 0
        host=(os.uname())[1]
        os.system("top -b -u %s  -n 1 > dbgtmp%s%d" % (os.environ.get("USER"),host,iproc))
        os.system("grep -i dddas dbgtmp%s%d > dbg%s%d" % (host,iproc,host,iproc))
...
if(idb):
  os.system( "idb -pid %d %s/exec/dddas_%s " %  
             (PID,os.environ.get("WORK"),os.environ.get("PETSC_ARCH")) )
else:
  os.system( "gdb %s/exec/dddas_%s %d" %  
